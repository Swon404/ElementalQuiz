...
export const EXTRA_FACTS = [
  ...
  "The phosphorus in a safety match is on the box’s striking strip, not in the match head.",
...
export function pickExtraFact(avoidText: string, used = new Set<string>()): string {
  const avoid = words(avoidText);
  const fresh = EXTRA_FACTS.filter(fact => !used.has(fact));
  const candidates = fresh.length ? fresh : [...EXTRA_FACTS];
  // Prefer a new topic over another version of the fact just explained.
...
export function addExtraFacts<T extends { explanation: string; questionText?: string; clues?: string[] }>(questions: T[]): (T & { extraFact: string })[] {
  const used = new Set<string>();
  return questions.map(question => ({ ...question, extraFact: pickExtraFact(`${question.questionText ?? question.clues?.join(' ') ?? ''} ${question.explanation}`, used) }));
}
